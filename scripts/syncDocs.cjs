...
const paths = {
  ...
  readmeMatching: path.join(__dirname, '../docs/README_CV_Matching_Parsing.md'),
  readmeInterview: path.join(__dirname, '../docs/README_Interview_System.md'),
...
`;
  ...
  fs.writeFileSync(paths.readmeMatching, markdown, 'utf8');

  // Also write to rules folder with a warning header
...
${markdown}`;
  ...
  console.log('[AutoDoc] CV Matching & Parsing README generated successfully in both docs and rules folders.');
}
...
`;
  ...
  fs.writeFileSync(paths.readmeInterview, markdown, 'utf8');
...
${markdown}`;
  ...
  console.log('[AutoDoc] Interview System README generated successfully in both docs and rules folders.');
}

function runSync() {
  try {
    const docsDir = path.dirname(paths.readmeMatching);
    ...
    console.log('[AutoDoc] All READMEs successfully updated in docs and rules folders!');
  } catch (err) {
